...
from scorer.task5 import evaluate
from format_checker.task5 import check_format
...
def run_baselines(lang='English'):
    ...
    gold_data_folder = join(ROOT_DIR, 'data/task5/{}'.format(lang))

    train_debates = [join(gold_data_folder, 'Task5-{}-1st-Presidential.txt'.format(lang)),
                     join(gold_data_folder, 'Task5-{}-Vice-Presidential.txt'.format(lang))]
    test_debate = join(gold_data_folder, 'Task5-{}-2nd-Presidential.txt'.format(lang))

    random_baseline_fpath = join(ROOT_DIR, 'baselines/data/task5_random_baseline_{}.txt'.format(lang))
    ...
    ngram_baseline_fpath = join(ROOT_DIR, 'baselines/data/task5_ngram_baseline_{}.txt'.format(lang))
